...
DeviceManager.prototype.loadDriver = function(manufId, driverName) {
	...
	if(!this.drivers[driverName]) {
		this.drivers[driverName] = require('./devices/'+manufId+'/'+driverName);
	}
...
DeviceManager.prototype.registerDevice = function(innerDev) {
	...
	if(!deviceType.value) {
		...
		device = this.getDeviceById(ieeeAddr);
		if(!device) {
			...
			if(driver) {
				...
				if(device.zclIncoming) { // Fuck my mind hack
					this.zap.controller.on(device.zclIncoming, device._zclHandle.bind(device));
					device.on('_zclHandle', function(data) {
			...
			// Update listeners
			/*device.removeAllListeners('reportAttribute');
			if(device.zclIncoming) {
				this.zap.controller.removeListener(device.zclIncoming, device._zclHandle);
				device.removeAllListeners('_zclHandle');
				this.zap.controller.on(device.zclIncoming, device._zclHandle.bind(device));
				device.on('_zclHandle', function(data) {
					this.zap.emit('ind', { type: 'devChange', data: data });
				}.bind(this));
			...
			if(device.isIas()) { // Restore enroll
				device.init();
			}
...
// DS_LEAVE
DeviceManager.prototype.unregisterDevice = function(id) {
	var devInfo, device = this.getDeviceById(id);
	if(device) {
		devInfo = device.info();
		if(device.zclIncoming) {
			this.zap.controller.removeAllListeners(device.zclIncoming, device._zclHandle);
		}
		device.cleanup();
		delete this.devices[id];
		log.info('Device %s is unregistered', id);
		this.zap._devbox.maintain(function(err) {
			if(err) log.error('Busybox', err);
		});
	}
	return devInfo||null;
}

// DS_OFFLINE / DS_ONLINE / DS_CHANGE_VALUE
DeviceManager.prototype.updateDevice = function(devupd) {
	var devInfo = null,
		id = _.isObject(devupd) ? devupd.ieeeAddr : devupd,
		device = this.getDeviceById(id);
	if(device) {
		var innerDev = this.zap.findDevByAddr(id); // DS_LEAVE innerDev = null
		if(innerDev && innerDev.status == 'online') {
			if(device.isIas() && device.zoneState == 0) {
				device.enroll(device.endpoint, 0x0).then(function() {
					log.info('Device %s enrolled successful', device.getGuid());
				}).done();
			}
		}
		device.dev = innerDev;
		if(_.isObject(devupd) && devupd.data) {
			devInfo = device.update(devupd.data);
		} else {
			devInfo = device.info();
		}
	}
...
DeviceManager.prototype.getDeviceInfoById = function(id) {
	var devInfo, device = this.getDeviceById(id);
	if(device){devInfo = device.info();}
	return devInfo||null;
}
...
DeviceManager.prototype.checkOnline = function() {
	...
	_.each(this.devices, function(device) {
		if(device.isIas() && device.zoneState) return; // If enrolled, they are in sleep mode
