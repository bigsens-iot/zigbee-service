		...
		if(!this.devices[ieeeAddr]) {
			...
			if(driver) {
				...
				if(device.zoneStatusNotification) { // Fuck my mind hack
					this.zap.controller.on(device.zoneStatusEvent, device.zoneStatusNotification.bind(device));
...
DeviceManager.prototype.unregisterDevice = function(id) {
	...
	if(device) {
		devInfo = device.info();
		if(device.zoneStatusEvent) {
			this.zap.controller.removeAllListeners(device.zoneStatusEvent, device.zoneStatusNotification);
		}
		device.cleanup();
