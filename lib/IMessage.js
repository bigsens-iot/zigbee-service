...
IMessage.prototype.serviceAnnounce = function() {
	return this.sendMessage('SERVICE_ANNCE', this.info());
}

IMessage.prototype.serviceReady = function() {
	return this.sendMessage('SERVICE_READY', { guid : this.getGuid() });
}

IMessage.prototype.serviceInfo = function() {
	return this.sendMessage('SERVICE_INFO', this.getNwkInfo());
}
...
IMessage.prototype.deviceState = function(state) {
	return this.sendMessage('DEVICE_STATE', state);
}
...
IMessage.prototype.on('message:DEVICE_WRITE_ATTRIBUTE', function(id, cls, attr, value) {
	var message = 'DEVICE_WRITE_ATTRIBUTE';
	if(this.isReady && this.deviceManager) {
		var device = this.deviceManager.getDeviceById(id);
		if(device) {
			var elist = device.innerDev.epList,
				ep = (elist.length == 1 ? elist[0] : null);
			if(ep) {
				device.writeAttribute(ep, cls, attr, value).then(function() {
					this.sendMessage(message, { device : this });
				}).fail(function(err) {
					this.sendMessage(message, { error : err });
				}).done();
...
IMessage.prototype.on('messages:MESSAGE_DISCOVER', function() {
	if(this.isReady && this.zap) {
		this.sendMessage('MESSAGE_DISCOVER', this._messageRegistry);
	}
});
...
IMessage.prototype.on('DEVICE_LIST', function() {
	debug('device list');
	if(this.isReady && this.deviceManager) {
		var deviceList = this.deviceManager.deviceList();
		
		console.log('DEVICE_LIST =', deviceList);
