...
DeviceManager.prototype.registerDevice = function(device) {
	...
	if(!this.devices[device.ieeeAddr]) {
		console.log('dev key =', deviceType.key);
		var drv = mapDrivers[deviceType.key];
		if(drv) {
			this.devices[device.ieeeAddr] = new drv(device);
			console.log('Device type %s registered %s', deviceType.key, fmtJson(device.dump()));
		} else {
...
DeviceManager.prototype.unregisterDevice = function(addr) {
	console.log('Device %s unregistered', addr);
