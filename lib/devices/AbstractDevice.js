...
AbstractDevice.prototype.invokeMethod = function(endpoint, cluster, cmd, params, callback) {
	var deferred = Q.defer(),
		params = params || {};
	if(this.dev && this.dev.status == 'online') {
		endpoint.functional(cluster, cmd, params).then(function(rsp) {
			deferred.resolve(this.dev, cluster, cmd, rsp);
		}.bind(this)).fail(function(err) {
...
AbstractDevice.prototype.cleanup = function() {
	...
	if(this.zoneStatusNotification) {
		this.removeAllListeners('zoneStatusNotification', this.zoneStatusNotification);
	}
	if(this.clearPolls) {
		this.clearPolls();
	}
