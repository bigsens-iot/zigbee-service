...
IASZoneDevice.prototype.readZoneState = function() {
	if(this.dev.status == 'online') {
		this.ep.foundation('ssIasZone', 'read', [ { attrId: 0x0002 } ])
			.then(function(rsp) {
				debug('ssIasZone.zoneState = %j', rsp);
				if(rsp.length === 1 && rsp[0].attrData) {
					this.parseState(rsp[0].attrData);
					if(this.zoneState.Alarm1) {
...
function ContactSwitchSensor(dev) {
	...
	setInterval(this.readZoneState.bind(this), 2000);
...
function FireSensor(dev) {
	...
	setInterval(this.readZoneState.bind(this), 5000);
